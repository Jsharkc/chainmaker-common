...
func TestBirdsNestImpl_Add(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "正常流 添加后存在",
			fields: fields{bn: getTBN(TestDir+"add0", t)},
			args:   args{key: GetTimestampKey()},
			want:   true,
		},
		{
			name:   "正常流 添加后不存在",
			fields: fields{bn: getTBN(TestDir+"add1", t)},
			args:   args{key: GetTimestampKey()},
			want:   true,
		},
	}
...
func TestBirdsNestImpl_Serialize(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "正常流",
			fields:  fields{bn: getTBN(TestDir+"Serialize", t)},
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := tt.fields.bn.Serialize(); (err != nil) != tt.wantErr {
...
func getTBN(path string, t *testing.T) *BirdsNestImpl {
	config := &common.BirdsNestConfig{
		...
		Snapshot: &common.SnapshotSerializerConfig{
			...
			Timed:       &common.TimedSerializeIntervalConfig{Interval: 5},
			BlockHeight: &common.BlockHeightSerializeIntervalConfig{Interval: 5},
			Path:        path,
		},
	}
...
func TestBirdsNestImpl_ValidateRule(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				rules: func() map[common.RuleType]Rule {
					m := make(map[common.RuleType]Rule)
					m[common.RuleType_AbsoluteExpireTime] = NewAETRule(1)
					return m
				}(),
			},
			args: args{
				key:   GetTimestampKey(),
				rules: []common.RuleType{common.RuleType_AbsoluteExpireTime},
			},
